...
//	Make a new tree from a list of points.
    ...
    //	Make a new tree from a list of points and specify numer of point to take
    public static KDTree MakeFromPoints(int count, params Vector3[] points)
    {
        int[] indices = Iota(points.Length);
        return MakeFromPointsInner(0, 0, System.Math.Min(count,points.Length - 1), points, indices);
    }
    ...
    static KDTree MakeFromPointsInner(
        ...
        root.pointCount = enIndex - stIndex;
...
//	Find the nearest point in the set to the supplied point.
	...
	public float FindNearest_R(Vector3 pt) {
		...
		return ((float)System.Math.Sqrt(bestSqDist));
	}	
...
// Find and returns all k neighbours	
    ...
    public List<int> FindNearestsRadius(Vector3 pt, float radius)
    {
        // Find and returns all k neighbours	
        float bestSqDist = 1000000000f;
        float minSqDist = 0f;
        int bestIndex = -1;
        List<int> bestIndexK = new List<int>();

        for (int i = 0; i <pointCount; i++)
        {
            SearchK(pt, ref bestSqDist, ref minSqDist, ref bestIndex);

            if (bestSqDist > radius) return bestIndexK;

            bestIndexK.Add(bestIndex);
...
// Find and returns all k neighbours distances
		...
		for(int i=0; i<k-1; i++){
			SearchK(pt, ref bestSqDist, ref minSqDist, ref bestIndex);
		    bestDistances[i] = (float)System.Math.Sqrt(bestSqDist);
		...
		SearchK(pt, ref bestSqDist, ref minSqDist, ref bestIndex);
		bestDistances[k-1] = (float)System.Math.Sqrt(bestSqDist);
